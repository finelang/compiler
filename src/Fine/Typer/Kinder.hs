...
kindedBlock :: Block Transformed -> REC KindEnv Error () (Block Kinded)
...
kindedExpr :: Expr Transformed -> REC KindEnv Error () (Expr Kinded)
...
kindedExpr (GenApp r f types) = do
  f' <- kindedExpr f
  kindEnv <- ask
  types' <- forM types $ \type' -> do
    let (type'', errs) = runKindChecker kindEnv type'
    forM_ errs fail'
    return type''
  return $ GenApp r f' types'
...
kindedExprBind :: Bind OfExpr Transformed -> REC KindEnv Error () (Bind OfExpr Kinded)
...
runKinder mdule = let (result, _) = runREC (kindedModule mdule) () in (result, [])
