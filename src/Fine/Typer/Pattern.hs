...
testCases matchedType = do
  de <- deepEnough
  if de
    then return [arbitraryTestCase]
    else deeper $ case matchedType of
      LiteralT _ litType -> return $ litTestCases litType
      VoidT _ -> return []
      TupleT _ fst' snd' rest -> do
        ...
        return $ TupleP NoRange <$> fstTestCases <*> sndTestCases <*> restTestCases
      ListT _ _ ->
      ...
      RecordT _ propTypes -> do
        propTestCases <- fmap combinations $ forM propTypes $ \(prop, type') -> do
          ...
          return $ map ((,) prop) cases
        return $ RecordP NoRange <$> propTestCases
      FunT _ _ _ ->
