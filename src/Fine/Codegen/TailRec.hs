...
-- -- TODO: reduce traversals
-- -- refactor to work with multiple substts using a `Map Id Id`
-- replaceIn :: Expr' -> Reader Substt Expr'
-- replaceIn expr@(Literal{}) = pure expr
...
--   (Substt old new) <- ask
--   pure (if var == old then Var ext new else expr)
-- replaceIn (App ext f args) = App ext <$> replaceIn f <*> mapM replaceIn args
-- replaceIn (GenApp ext f typeArgs) = GenApp ext <$> replaceIn f <*> pure typeArgs
-- replaceIn (Access ext expr prop) = Access ext <$> replaceIn expr <*> pure prop
-- replaceIn (Index ext expr ix) = Index ext <$> replaceIn expr <*> pure ix
-- replaceIn (Cond ext cond yes no) =
--   Cond ext <$> replaceIn cond <*> replaceIn yes <*> replaceIn no
-- replaceIn fun@(Fun ext params body) = do
--   old <- asks oldVar
--   if old `elem` params
--     then pure fun
--     else Fun ext params <$> replaceIn body
-- replaceIn (GenFun ext typeParams body) = GenFun ext typeParams <$> replaceIn body
...
--   inBlock (Mut var expr block) = do
--     expr' <- replaceIn expr
--     (Substt old new) <- ask
--     let var' = if var == old then new else var
--     block'' <- inBlock block
--     pure (Mut var' expr' block'')
--   inBlock (Let isMut binder expr block) = do
--     old <- asks oldVar
--     let blockAction = if old == binder then pure else inBlock
--     Let isMut binder <$> replaceIn expr <*> blockAction block
--   inBlock block@Void = pure block
...
-- tryTransformBlock f (Mut var expr block) = Mut var expr <$> tryTransformBlock f block
...
--               branches
--               recsTransformed
--        in pure (PatternMatching ext expr (NonEmpty.zip patts allTransformed))
...
--               branches
--               recsTransformed
--        in pure (Cond ext cond (allTransformed !! 0) (allTransformed !! 1))
...
--   pure (foldr (Fun invalidX) (Block invalidX block) params)
